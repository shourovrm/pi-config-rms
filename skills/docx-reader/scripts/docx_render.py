#!/usr/bin/env python3
"""Render DOCX pages to PNG images by converting to PDF first, then rasterizing.

Requires LibreOffice for the DOCX → PDF conversion step.
"""
...
def render(path: str, page_spec: str = "all", dpi: int = 150) -> list[str]:
    ...
    if not lo_bin:
        ...
        print("  sudo apt install libreoffice-core libreoffice-writer", file=sys.stderr)
        print("  or: brew install --cask libreoffice", file=sys.stderr)
        sys.exit(1)

    abs_path = os.path.abspath(path)
    work_dir = tempfile.mkdtemp(prefix="pi-docx-")

    print(f"Converting DOCX → PDF via LibreOffice...", file=sys.stderr)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Render DOCX pages to PNG via PDF conversion")
