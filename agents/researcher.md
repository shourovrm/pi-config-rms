...
2. **Search** - use `web_search` with varied angles. Search for the latest available information as of <early|mid|late> <year>. Return an answer with source URLs.`
3. **Evaluate** - Read the answers. Identify what's well-covered, what has gaps, what's noise
4. **Deep dive** - For the 2-3 most promising source URLs, use `web_fetch` to get full page content
